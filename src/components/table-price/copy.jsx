  ...
  useEffect(() => {
    ...
    try {
      ...
      const calc = (tier, sides, type, size) => {
        // Логіка множника друку як у твоєму початковому коді копіювання
        const baseMult = size === "A3" ? tier.count : tier.count * 0.5;
        const printingData = data.PRINTING?.[type];
        const scanData = data.SCAN?.AUTO?.[size];

        if (!printingData || !scanData) return "0.00";

        // Розрахунок: (Друк + Скан) / кількість + папір
        const printingPrice = getPrintingPrice(baseMult, sides, printingData);
        const scanPrice = getScanPrice(tier.count * sides, 1, scanData);

        const price =
          ((printingPrice + scanPrice) / tier.count + paperPrice) * rate;
      ...
      console.error("Copy calculation error:", error);
    }
  }, [data, valute]);
      ...
      <TableCard>
        ...
        <TableWrapper>
          <StyledTable>
            ...
            <div className="row-title">{t("copy.h3_1sided")}</div>
            {prices[activeSize].map((p, i) => (
              <div key={i} className="price-cell">
                {formatter.format(p.c1)}
              </div>
            ))}
            <div className="row-title">{t("copy.h3_2sided")}</div>
            {prices[activeSize].map((p, i) => (
              <div key={i} className="price-cell">
        ...
        <Note>{t("copy.span")}</Note>
      </TableCard>
    </Container>
  );
};

export default TablePriceCopy;
